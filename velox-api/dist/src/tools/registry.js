...
// ─── HTTP helpers ─────────────────────────────────────────────────────────────
const REQUEST_TIMEOUT_MS = 8000; // 8 s — tight deadline for voice UX
...
exports.toolRegistry = {
    ...
    check_order_status: async (args) => {
        logger_1.logger.info({ tool: "check_order_status", order_id: args.order_id });
        const baseUrl = process.env.ORDER_API_URL;
        if (!baseUrl) {
            logger_1.logger.warn("ORDER_API_URL not set — returning service-unavailable");
            return { error: "Order service is currently unavailable. Please try again later." };
        }
        try {
            const data = await apiGet(`${baseUrl}/orders/${encodeURIComponent(args.order_id)}`, process.env.ORDER_API_KEY
            ...
            logger_1.logger.error({ err: err.message, tool: "check_order_status" }, "API call failed");
    ...
    check_item_stock: async (args) => {
        logger_1.logger.info({ tool: "check_item_stock", item_name: args.item_name });
        const baseUrl = process.env.INVENTORY_API_URL;
        if (!baseUrl) {
            logger_1.logger.warn("INVENTORY_API_URL not set — returning service-unavailable");
            return { error: "Inventory service is currently unavailable. Please try again later." };
        }
        try {
            const data = await apiGet(`${baseUrl}/products?name=${encodeURIComponent(args.item_name)}`);
            ...
            logger_1.logger.error({ err: err.message, tool: "check_item_stock" }, "API call failed");
            return { error: `Could not check stock for "${args.item_name}". Please try again later.` };
        }
    },
    // ── book_appointment ────────────────────────────────────────────────────────
    book_appointment: async (args) => {
        logger_1.logger.info({ tool: "book_appointment", ...args });
        const baseUrl = process.env.CALENDAR_API_URL;
        if (!baseUrl) {
            logger_1.logger.warn("CALENDAR_API_URL not set — returning service-unavailable");
        ...
        try {
            ...
            logger_1.logger.error({ err: err.message, tool: "book_appointment" }, "API call failed");
    ...
    search_faq: async (args) => {
        logger_1.logger.info({ tool: "search_faq", question: args.question.substring(0, 80) });
        const faqKbId = process.env.FAQ_KB_ID;
        if (!faqKbId) {
            logger_1.logger.warn("FAQ_KB_ID not set — FAQ search disabled");
            return {
                found: false,
        ...
        try {
            ...
            logger_1.logger.error({ err: err.message, tool: "search_faq" }, "RAG call failed");
            return {
                found: false,
    ...
    get_customer_profile: async (args) => {
        logger_1.logger.info({ tool: "get_customer_profile", customer_id: args.customer_id, phone: args.phone_number });
        const baseUrl = process.env.CRM_API_URL;
        if (!baseUrl) {
            logger_1.logger.warn("CRM_API_URL not set — returning service-unavailable");
        ...
        try {
            ...
            logger_1.logger.error({ err: err.message, tool: "get_customer_profile" }, "API call failed");
            return { found: false, error: "Could not retrieve customer profile. Please try again later." };
        }
    },
    // ── trigger_human_handoff ───────────────────────────────────────────────────
    trigger_human_handoff: async (args) => {
        logger_1.logger.info({ tool: "trigger_human_handoff", reason: args.reason, priority: args.priority });
        const baseUrl = process.env.HANDOFF_API_URL;
        if (!baseUrl) {
            // Graceful degradation — still signal handoff intent even without API
            logger_1.logger.warn("HANDOFF_API_URL not set — returning default handoff response");
        ...
        try {
            ...
            return {
                ...
                message: `I'm connecting you to a live agent now. ` +
                    `Estimated wait time is ${data.estimated_wait ?? "a few minutes"}. ` +
                    `Reason: ${args.reason}. Please hold.`,
            };
        }
        catch (err) {
            logger_1.logger.error({ err: err.message, tool: "trigger_human_handoff" }, "API call failed");

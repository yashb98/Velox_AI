 ...
 */
async function ensureHnswIndex() {
    try {
      ...
      `);
        logger_1.logger.info("HNSW index ensured on knowledge_chunks.embedding");
    }
    catch (err) {
        ...
        logger_1.logger.warn({ err: err.message }, "HNSW index creation skipped (non-fatal)");
    }
}
// ─── SHA-256 helper ──────────────────────────────────────────────────────────
/**
...
class IngestionService {
    ...
    async ingestDocument(kbId, content, metadata = {}) {
        try {
            ...
            logger_1.logger.info(`Split into ${chunks.length} chunks`);
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                if (!embedding) {
                    logger_1.logger.warn(`Skipping chunk ${i + 1} (embedding failed)`);
                    continue;
                }
          ...
          `);
                if (existing.length > 0) {
                    logger_1.logger.debug(`Chunk ${i + 1} already exists (content_hash match) — skipping`);
                    skippedCount++;
                    continue;
                }
          ...
          `);
            ...
            logger_1.logger.info({ created: successCount, skipped: skippedCount, total: chunks.length }, "Ingestion complete");

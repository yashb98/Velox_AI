 ...
 */
router.post("/incoming", twilioAuth_1.validateTwilioWebhook, async (req, res) => {
    ...
    if (!agent) {
        logger_1.logger.warn({ To, CallSid }, "Incoming call — no agent found for number");
    ...
    if (agent.org.credit_balance < MIN_REQUIRED) {
        logger_1.logger.warn({ orgId: agent.org_id, balance: agent.org.credit_balance, CallSid }, "Call rejected — insufficient credit balance");
        const twiml = new VoiceResponse_1.default();
        twiml.say("We are unable to take your call at this time. Please try again later.");
        res.type("text/xml");
        return res.send(twiml.toString());
    }
    ...
    try {
        ...
        logger_1.logger.warn({ err, CallSid }, "CallReservation already exists — skipping create");
    }
    ...
    logger_1.logger.info({ CallSid, agentId: agent.id, orgId: agent.org_id, conversationId: conversation.id }, "Incoming call accepted — connecting to Media Stream");
